...
class ModelHandler(object):
	...
	def train(self):
		...
		if args.model == 'PCGNN':
			...
			agg_gcn = GCNAggregator(features, cuda=args.cuda, gat = False)
			enc_gcn = GCNEncoder(features, feat_data.shape[1], args.emb_size, adj_lists, agg_gcn, gcn=True, cuda=args.cuda)
		elif args.model == 'GAT':
			agg_gat = GCNAggregator(features, cuda=args.cuda, gat = True)
			enc_gat = GCNEncoder(features, feat_data.shape[1], args.emb_size, adj_lists, agg_gat, cuda=args.cuda)
