...
# ------------------My code---------------------------------
		...
		self.out_features = 2*feat_dim #之后可以改，调参的一部分
		...
		for i in range(self.n_heads):
			self.W.append(nn.Parameter(torch.empty(size=(self.in_features, self.out_features))))
			nn.init.xavier_uniform_(self.W[i].data, gain=1.414)
			self.a.append(nn.Parameter(torch.empty(size=(2*self.out_features, 1))))
	...
	def _prepare_attentional_mechanism_input(self, Wh1, Wh2, a):
        ...
        # Wh1&2.shape (N, 1)
        # e.shape (N, N)
		Wh1 = torch.matmul(Wh1, a[:self.out_features, :])
		Wh2 = torch.matmul(Wh2, a[self.out_features:, :])
        # broadcast add
		e = Wh1 + Wh2.T
...
# 分别获得 目标点 和 源点 的特征矩阵
# 对每个特征矩阵高维投影之后，传入函数 返回注意力矩阵（未过滤）
		...
		for i in range(self.n_heads):
			Wh1.append(torch.mm(self_feats, self.W[i])) # h.shape: (N, in_features), Wh.shape: (N, out_features)
			Wh2.append(torch.mm(embed_matrix, self.W[i]))
			E.append(self._prepare_attentional_mechanism_input(Wh1[i], Wh2[i], self.a[i]))
        


# -----------------------------------------------------------------

# 用原PC-GNN代码 创建邻接矩阵，用于过滤刚才的注意力矩阵

		# intra-relation aggregation only with sampled neighbors
		raw_mask = Variable(torch.zeros(len(samp_neighs), len(unique_nodes)))
		#print(raw_mask.shape)

		column_indices = [unique_nodes[n] for samp_neigh in samp_neighs for n in samp_neigh]
		row_indices = [i for i in range(len(samp_neighs)) for _ in range(len(samp_neighs[i]))]
		raw_mask[row_indices, column_indices] = 1
        
# -------------------------My code---------------------------------

# 过滤（*意思是矩阵相应位置乘积）
		zero_vec = -9e6*torch.ones_like(raw_mask)
		sub_mask = []
		for i in range(self.n_heads):
			e = E[i]
			attention = torch.where(raw_mask > 0, e, zero_vec)
			attention = F.softmax(attention, dim=1)
			#attention = F.dropout(attention, self.dropout, training=self.training)
			sub_mask.append(attention)
            
#h_prime = torch.matmul(attention, Wh)		
		sum_mask = Variable(torch.zeros(len(samp_neighs), len(unique_nodes)))
		for i in range(self.n_heads):
			sum_mask = sum_mask + sub_mask[i]
		
		final_mask = sum_mask/self.n_heads
# -----------------------------------------------------------------
...
#		mask = mask.div(num_neigh)  # mean aggregator
		if self.cuda:
			...
			self_feats = self.features(torch.LongTensor(nodes))
			embed_matrix = self.features(torch.LongTensor(unique_nodes_list))
		agg_feats = final_mask.mm(embed_matrix)  # single relation aggregator
